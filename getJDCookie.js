...
function loginEntrance() {
    ...
    $.post(taskPostUrl(), (err, resp, data) => {
      try {
        if (err) {
          ...
          data = JSON.parse(data);
          token = data['token'];
          // $.log('token', token)          const setCookie = resp.headers['set-cookie'][0];
  ...
  new Promise(resolve => {
    ...
    const cookie1 = "pt_key=" + pt_key + ";pt_pin=" + pt_pin + ";";    $.UserName = decodeURIComponent(cookie1.match(/pt_pin=(.+?);/) && cookie1.match(/pt_pin=(.+?);/)[1])
