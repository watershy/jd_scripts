...
[task_local]
# 京东赚赚
10 0 * * * https://gitee.com/lxk0301/jd_scripts/raw/master/jd_jdzz.js, tag=京东赚赚, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jdzz.png, enabled=true================Loon==============
[Script]
cron "10 0 * * *" script-path=https://gitee.com/lxk0301/jd_scripts/raw/master/jd_jdzz.js,tag=京东赚赚===============Surge=================
京东赚赚 = type=cron,cronexp="10 0 * * *",wake-system=1,timeout=3600,script-path=https://gitee.com/lxk0301/jd_scripts/raw/master/jd_jdzz.js============小火箭=========
...
async function jdWish() {
  ...
  // }
  // if ($.tuan) $.tuanList.push($.tuan)  await helpFriends()
  await getUserInfo()
  await getTaskList()
...
function showMsg() {
  return new Promise(async resolve => {
    ...
    // 云端大于10元无门槛红包时进行通知推送
    if ($.isNode() && $.totalNum >= 100000) await ck.methodEnd($, `京东账号${$.index} ${$.nickName}\n当前金币：${$.totalNum}个\n可兑换无门槛红包：${parseInt($.totalNum) / 10000}元\n`,)
    resolve();
  })
}
