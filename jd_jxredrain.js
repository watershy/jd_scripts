        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data['ret'] === 0) {
              ...
              const helpCount = assistNumMax-assistListToday.length
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data['iRet'] === 0) {
              data = data['Data'];
              $.shareCode = data.strUserPin
              //"K_WPuc5yI-uG48XSS6bhrVwJLTV8t3-kxhwqY1m7pN6z_C1zcVFyv_qffDK1JRvP"
            } else {
...
function taskurl(functionId, body = '') {
  return {
    url: `${JD_API_HOST}//cubeactive/steprewardv3/${functionId}?activeId=470098&publishFlag=1&channel=7&timestamp=${(new Date()).getTime()}&userDraw=1&_=${(new Date().getTime())}&sceneval=2&g_login_type=1`,
    headers: {
      'Cookie': cookie,
      'Host': 'wq.jd.com',
      'Accept': '*/*',
