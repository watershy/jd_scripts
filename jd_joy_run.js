/**
 ...
 [MITM]
 hostname = draw.jdfcloud.com===========Surge=================
 [Script]
 ...
 宠汪汪助力获取Token = type=http-request,pattern=^https:\/\/draw\.jdfcloud\.com(\/mirror)?\/\/api\/user\/user\/detail\?openId=, max-size=0, script-path=https://gitee.com/lxk0301/jd_scripts/raw/master/jd_joy_run.js===================Quantumult X=====================
 [task_local]
 # 宠汪汪邀请助力与赛跑助力
 ...
 # 宠汪汪助力获取Token
 ^https:\/\/draw\.jdfcloud\.com(\/mirror)?\/\/api\/user\/user\/detail\?openId= url script-request-header https://gitee.com/lxk0301/jd_scripts/raw/master/jd_joy_run.js=====================Loon=====================
 [Script]
...
//此处填入你需要助力好友的京东用户名
//下面给出好友邀请助力的示例填写规则
let invite_pins = ["jd_pBXzZlqInyyk"];
//下面给出好友赛跑助力的示例填写规则
let run_pins = ["jd_pBXzZlqInyyk"];
...
async function getToken() {
  ...
  if (isURL(url, /^https:\/\/draw\.jdfcloud\.com(\/mirror)?\/\/api\/user\/addUser\?code=/)) {
    ...
    if ($request && $request.method !== 'OPTIONS') {
      ...
      //}
      $.setdata(LKYLToken, 'jdJoyRunToken');
      $.msg($.name, '获取Token: 成功🎉', ``);      // $.done({ body: JSON.stringify(body) })
      $.done({url: url})
    }
  } else {
    $.done()
