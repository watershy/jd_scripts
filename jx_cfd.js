...
!(async () => {
  ...
  for (let i = 0; i < $.cookieArr.length; i++) {
    ...
    if ($.currentCookie) {
      ...
      $.log(`\n开始【京东账号${i + 1}】${userName}`);      const beginInfo = await getUserInfo();
      // if (i === 0) {
      //   存储第一个账号的shareCode到文件里面
        // await fs.writeFileSync('./cfdShareCode.txt', beginInfo.strMyShareId, 'utf8');
      // }
      await $.wait(500);
      await querySignList();      await $.wait(500);
      await getMoney();      //日常任务
      await $.wait(500);
      await getTaskList(0);
      await $.wait(500);
      await browserTask(0);      //寻宝
      await $.wait(500);
      await treasureHunt();      //偷财富
      await $.wait(500);
      await friendCircle();      //成就任务
      await $.wait(500);
      await getTaskList(1);
      await $.wait(500);
      await browserTask(1);      //抽奖
      await $.wait(500);
      await funCenterState();      const endInfo = await getUserInfo();
      $.result.push(
      ...
      //本地存储
      // await $.wait(500);
      // const shareCode = await fs.readFileSync('./cfdShareCode.txt', "utf8");
      //超级助力
      // await $.wait(500);
      // await createSuperAssistUser(shareCode);
      // 普通助力
      // await $.wait(500);
...
//随机超级助力好友
function createSuperAssistUser(shareCode) {
  return new Promise(resolve => {
    const sceneIds = Object.keys($.info.SceneList);
    const sceneId = Math.min(...sceneIds);
      try {
        $.log(`\n【👫🏻超级助力】超级助力码：${shareCode}`);
        $.get(taskUrl('user/JoinScene', `strPgtimestamp=${$.currentToken['timestamp']}&strPhoneID=1a332c2cd0458a16-35355334688244&strPgUUNum=${$.currentToken['farm_jstoken']}&strShareId=${escape(shareCode)}&dwSceneId=${sceneId}&dwType=2`), async (err, resp, data) => {
          try {
