...
[task_local]
#宠汪汪积分兑换奖品
0 0-16/8 * * * https://gitee.com/lxk0301/jd_scripts/raw/master/jd_joy_reward.js, tag=宠汪汪积分兑换奖品, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jdcww.png, enabled=true==============Loon==============
[Script]
cron "0 0-16/8 * * *" script-path=https://gitee.com/lxk0301/jd_scripts/raw/master/jd_joy_reward.js,tag=宠汪汪积分兑换奖品================Surge===============
宠汪汪积分兑换奖品 = type=cron,cronexp="0 0-16/8 * * *",wake-system=1,timeout=3600,script-path=https://gitee.com/lxk0301/jd_scripts/raw/master/jd_joy_reward.js===============小火箭==========
                ...
                if (leftStock) {
                    ...
                    if ($.exchangeRes && $.exchangeRes.success) {
                        if ($.exchangeRes.errorCode === 'buy_success') {
                            ...
                            $.notice += `----------------------------\n`
                            $.notice += `【京东账号${$.index}】 ${$.nickName}\n兑奖异常:${JSON.stringify($.exchangeRes)}`
                        }
                    } else {
                    ...
                    $.notice += `----------------------------\n`
                    $.notice += `【京东账号${$.index}】 ${$.nickName}\n兑换${rewardNum}京豆失败，原因：京豆库存不足，已抢完，请下一场再兑换`
                }
            } else {
                console.log(`兑换${rewardNum}京豆失败，原因：您目前只有${data.coin}积分，已不足兑换${giftValue}京豆所需的${salePrice}积分\n`)
                $.notice += `----------------------------\n`
                $.notice += `【京东账号${$.index}】 ${$.nickName}\n兑换${rewardNum}京豆失败，原因：您目前只有${data.coin}积分，已不足兑换${giftValue}京豆所需的${salePrice}积分\n`
            }
        } else {
